...
"""
function Base.shift!(seq::GeneralSequence)
    if isempty(seq)
        throw(ArgumentError("sequence must be non-empty"))
    end
    x = seq[1]
...
"""
function Base.unshift!(seq::GeneralSequence{A}, x) where {A}
    bin = enc64(seq, x)
    resize!(seq, length(seq) + 1)
...
"""
function complement(seq::GeneralSequence{A}) where {A<:NucleicAcidAlphabet}
...
function Base.shuffle(seq::GeneralSequence)

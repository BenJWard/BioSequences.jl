...
function Base.copy!(seq::GeneralSequence{A}, doff::Integer,
                    src::Vector{UInt8},  soff::Integer, len::Integer) where {A}
    datalen = seq_data_len(A, len)
    return seq
end

function Base.copy!(dst::GeneralSequence{A}, src::GeneralSequence{A}) where {A}
    return copy!(dst, 1, src, 1)
end

function Base.copy!(dst::GeneralSequence{A}, doff::Integer,
...
function Base.copy(seq::GeneralSequence{A}) where {A}
    newseq = GeneralSequence{A}(seq, 1:endof(seq))

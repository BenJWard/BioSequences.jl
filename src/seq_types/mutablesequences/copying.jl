...
function Base.copy!(seq::MutableBioSequence{A}, doff::Integer,
                    src::Vector{UInt8},  soff::Integer, len::Integer) where {A}
    datalen = seq_data_len(A, len)
    return seq
end

function Base.copy!(dst::MutableBioSequence{A}, src::MutableBioSequence{A}) where {A}
    return copy!(dst, 1, src, 1)
end

function Base.copy!(dst::MutableBioSequence{A}, doff::Integer,
...
function Base.copy(seq::MutableBioSequence{A}) where {A}
    newseq = MutableBioSequence{A}(seq, 1:endof(seq))
...
function encode_copy!(dst::MutableBioSequence{A},
    ...
    #println("Typeof stop: ", typeof(stop))
